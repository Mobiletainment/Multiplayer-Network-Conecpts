	...
	void DynamicActorReplicaComponent::serializeConstruction(RakNet::BitStream *constructionBitstream)	{
		...
		if(constructionDictionary != nullptr)	{
			...
			if(vehicle != nullptr)	{
				...
				constructionDictionary->setObject(CCFloat::create(localSpaceData._linearVelocity.x), "lvx");
				constructionDictionary->setObject(CCFloat::create(localSpaceData._linearVelocity.y), "lvy");
				constructionDictionary->setObject(CCFloat::create(localSpaceData._angularVelocity.z), "avz");
			}
