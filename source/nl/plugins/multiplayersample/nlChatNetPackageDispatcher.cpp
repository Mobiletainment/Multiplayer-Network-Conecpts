...
namespace nl
{
	...
	ChatNetPackageDispatcher::ChatNetPackageDispatcher() : _ctrlChatText(nullptr)
	{
	...
	AbstractChatNetPackageSender::AbstractChatNetPackageSender() : _autoMessageTimer(0.0f), _autoMessageInterval(5.0f), _autoMessageIdx(0)
	{
	...
	ClientChatNetPackageSender::ClientChatNetPackageSender() : _ctrlName(nullptr), _ctrlPassword(nullptr), _ctrlMessage(nullptr)
	{

		CCNotificationCenter* notificationCenter(CCNotificationCenter::sharedNotificationCenter());
		notificationCenter->addObserver(this, callfuncO_selector(ClientChatNetPackageSender::onCtrlEditEnd), SL_NOTIFY_CTRL_EDIT_END, NULL );
	}
...
#define SL_CTRLID_REGISTER 23000

	void ClientChatNetPackageSender::onUIAction( CCObject *sender, CCControlEvent controlEvent )
	{
	...
	void ClientChatNetPackageSender::createClientUI( PeerNode* peerNode, CCControlBase* parentLayer )
	{
		...
		*/
		...
		// Kristian Added Start - get layer size
		CCSize anSize = parentLayer->getContentSize();
		// Kristian Added End

		// Kristian Added Start - This part adds name, password edit-boxes and a register button to an container and adds the container to the according layer
		CCSize btnSize = CCSizeMake(anSize.width - 20.0f, 0);
		CCSize ctrlSize = CCSizeMake(anSize.width, anSize.height - 160.0f);

		_ctrlName = ControlUtils::createEditBox("Name:", btnSize);
		_ctrlPassword = ControlUtils::createEditBox("Password:", btnSize);

		CCControlButton* ctrlBtn;
		ctrlBtn = sl::ControlUtils::createButton("Register");
		ctrlBtn->setTag(SL_CTRLID_REGISTER);
		ctrlBtn->addTargetWithActionForControlEvents(this, cccontrol_selector(ClientChatNetPackageSender::onUIAction), CCControlEventTouchUpInside);
		ctrlBtn->setPreferredSize(btnSize);
		...
		CCControlColumn* ctrlContainer = ControlUtils::createControlColumn(ctrls, ctrlSize);
		ctrlContainer->setPositionY(160.0f);
	...
	void ClientChatNetPackageSender::sendData( const char* data, int lenght )
	{
		...
		{
			rakNetPeer->Send(data, lenght, HIGH_PRIORITY, RELIABLE_ORDERED, 0, target, false);
		}
	}
	...
	void ServerChatNetPackageSender::sendData(const char* data, int lenght )
	{
		...
		RakNet::RakPeerInterface* rakNetPeer(getPeer()->accessRakNetPeer());
		rakNetPeer->Send(data, lenght, HIGH_PRIORITY, RELIABLE_ORDERED, 0, target, true);
	}
