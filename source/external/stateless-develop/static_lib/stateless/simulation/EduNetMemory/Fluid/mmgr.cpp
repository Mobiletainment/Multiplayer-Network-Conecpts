...
// ---------------------------------------------------------------------------------------------------------------------------------
// -MODIFIED [JF]- 64bit fix and printing to stdout
// ---------------------------------------------------------------------------------------------------------------------------------
...
static	sAllocUnit	*findAllocUnit(const void *reportedAddress)
{
	...
	FMM_HashIndexType	hashIndex = (reinterpret_cast<FMM_HashIndexType>(const_cast<void *>(reportedAddress)) >> 4) & (hashSize - 1);
...
//	pattern = 0;
	...
	if (alwaysWipeAll && allocUnit->reportedSize > originalReportedSize)
	{
		...
		int	length = static_cast<int>(allocUnit->reportedSize - originalReportedSize);
                ...
                int	i;
                int iEnd = (length >> __MMGR_WIPESHIFT);
                for (i = 0; i < iEnd; i++, lptr++)
		{
			*lptr = pattern;
		}
...
static	void	dumpAllocations(FILE *fp)
{
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		while(ptr)
		{
			fprintf(fp, "%06d 0x%08X 0x%08d 0x%08X 0x%08d 0x%08X %-8s    %c       %c    %s\r\n",
//			fprintf(fp, "%06d 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X %-8s    %c       %c    %s\r\n",
				ptr->allocationNumber,
				reinterpret_cast<FMM_HashIndexType>(ptr->reportedAddress), ptr->reportedSize,
				reinterpret_cast<FMM_HashIndexType>(ptr->actualAddress), ptr->actualSize,
				m_calcUnused(ptr),
...
static	void	dumpLeakReport(FILE  *fp)
{
...
//MERGEME
//[cp] free the memory any ways !!!
//[cp] so moved to the bottom
...
// ---------------------------------------------------------------------------------------------------------------------------------
static  void    dumpLeakReportToFile( void )
{
...
static  void    dumpLeakReportToStdOut( void )
{
    FILE  *fp = stdout;
    dumpLeakReport( fp );
}
...
{
    dumpLeakReportToFile();
    dumpLeakReportToStdOut();
}
// ---------------------------------------------------------------------------------------------------------------------------------
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		FMM_HashIndexType	hashIndex = (reinterpret_cast<FMM_HashIndexType>(au->reportedAddress) >> 4) & (hashSize - 1);
		...
		if (alwaysLogAll) log("[+] ---->             addr 0x%08X", reinterpret_cast<FMM_HashIndexType>(au->reportedAddress));
...
void	*m_reallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, const size_t reportedSize, void *reportedAddress)
{
	try
	{
		...
		if (oldReportedAddress != au->reportedAddress)
		{
			...
			{
				FMM_HashIndexType	hashIndex = (reinterpret_cast<FMM_HashIndexType>(oldReportedAddress) >> 4) & (hashSize - 1);
			...
			hashIndex = (reinterpret_cast<FMM_HashIndexType>(au->reportedAddress) >> 4) & (hashSize - 1);
			if (hashTable[hashIndex]) hashTable[hashIndex]->prev = au;
			au->next = hashTable[hashIndex];
			au->prev = NULL;
			hashTable[hashIndex] = au;
...
void	m_deallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int deallocationType, const void *reportedAddress)
{
	try
	{
		...
		if (alwaysLogAll) log("[-] ----- %8s of addr 0x%08X           by %s", allocationTypes[deallocationType], reinterpret_cast<FMM_HashIndexType>(const_cast<void *>(reportedAddress)), ownerString(sourceFile, sourceLine, sourceFunc));
